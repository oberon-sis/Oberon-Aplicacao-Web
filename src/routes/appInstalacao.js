...
router.get('/windows/agente', (req, res) => {
  ...
  res.download(filePath, 'MonitoramentoOberonWindows.zip', (err) => {
    if (err) {
...
router.get('/linux/agente', (req, res) => {
  const downloadsPath = path.join(__dirname, '..', '..', 'downloads');
  const filePath = path.join(downloadsPath, 'Agente_Monitoramento_Linux.zip');

  res.download(filePath, 'MonitoramentoOberonLinux.zip', (err) => {
    if (err) {
      console.error('Erro ao tentar enviar o arquivo:', err);
      if (err.code === 'ENOENT') {
        return res.status(404).send('Arquivo do Agente não encontrado no servidor.');
...
router.get('/sh/agente', (req, res) => {
  const downloadsPath = path.join(__dirname, '..', '..', 'downloads');
  const filePath = path.join(downloadsPath, 'Agente_Monitoramento_Sh.zip');
  res.download(filePath, 'MonitoramentoOberonSh.zip', (err) => {
    if (err) {
      console.error('Erro ao tentar enviar o arquivo:', err);
      if (err.code === 'ENOENT') {
        return res.status(404).send('Arquivo do Agente não encontrado no servidor.');
