...
function buscar_info_componentes(idMaquina) {
  console.log('[MODEL] - Buscando componentes da máquina:', idMaquina);
...
function buscar_dados_kpi(idMaquina) {
  console.log('[MODEL] - Buscando KPIs da máquina:', idMaquina);
...
function buscar_parametros(idMaquina) {
  console.log('[MODEL] - Buscando parâmetros da máquina:', idMaquina);
...
function buscar_info_24_horas_coleta(idMaquina) {
  console.log('[MODEL] - Buscando dados de coleta 24h da máquina:', idMaquina);
  ...
  var instrucaoSql = `
        ...
        FROM Registro AS r
        JOIN vw_ComponentesParaAtualizar AS c
            ON r.fkComponente = c.idComponente
        WHERE c.fkMaquina = ${idMaquina}
          AND r.horario >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
        GROUP BY intervaloTempo, c.tipoComponente
        ORDER BY intervaloTempo ASC, c.tipoComponente ASC;
    `;
...
function buscar_dados_cards_painel(idEmpresa, pagina_atual, tamanho_da_pagina, statusFiltro, nome_busqueda) {
    console.log('[MODEL] - Buscando cards do painel', idEmpresa, pagina_atual, tamanho_da_pagina, statusFiltro, nome_busqueda);
...
function buscar_dados_filtros(idEmpresa) {
  console.log('[MODEL] - Buscando filtros do painel para empresa', idEmpresa);
