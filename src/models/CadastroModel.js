...
function buscarPorEmail(email) {
  ...
  var instrucaoSql = `
        SELECT f.*, DATE_FORMAT(e.dataCriacao, '%Y-%m-%d') AS DataCriacaoEmpresa, e.LinkCanalSlack FROM Funcionario as f join Empresa as e on f.fkEmpresa = e.idEmpresa WHERE email = '${email}';
    `;
  console.log('Executando a instrução SQL: \n' + instrucaoSql);
  return database.executar(instrucaoSql);
}
