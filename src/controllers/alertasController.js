...
function listarAlertas(req, res) {
  try {
    const { idUsuario, pagina, tipoFiltro, termoPesquisa, dataInicio, dataFim } =
      processarFiltros(req);

    alertasModel
      .getFkEmpresa(idUsuario)
      .then(function (resultadoEmpresa) {
        if (resultadoEmpresa.length === 0 || !resultadoEmpresa[0].fkEmpresa) {
          return res
            .status(404)
            .send(
              'Empresa do usuário não encontrada. Verifique se o Funcionario está vinculado a uma Empresa.',
            );
        }
        ...
        Promise.all([
          alertasModel.contarTotalAlertas(
            fkEmpresa,
            tipoFiltro,
            termoPesquisa,
            dataInicio,
          ...
          .then(function ([resultadoContagem, resultadoAlertas]) {
            ...
            if (resultadoAlertas.length > 0 || totalAlertas > 0) {
              ...
              res
                .status(204)
                .send(
                  'Nenhum alerta encontrado para esta empresa ou página solicitada está vazia.',
                );
            }
          })
          .catch(function (erro) {
  ...
  try {
    ...
    alertasModel
      ...
      .then(function (resultadoEmpresa) {
        ...
        const fkEmpresa = resultadoEmpresa[0].fkEmpresa;
        return alertasModel.obterTodosAlertasParaExportacao(
          fkEmpresa,
          tipoFiltro,
          termoPesquisa,
          dataInicio,
      ...
      .then(function (alertas) {
        if (alertas.length === 0) {
          return res
            .status(204)
            .send('Nenhum alerta encontrado para exportação com os filtros aplicados.');
        }
        const fields = [
