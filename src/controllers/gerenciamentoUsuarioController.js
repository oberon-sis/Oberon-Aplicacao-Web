...
function cadastrar(req, res) {
  ...
  if (!nome) {
    ...
    return res
      .status(400)
      .send("O campo CPF/Telefone está vazio ou indefinido!");
  } else if (fkEmpresa === undefined) {
    return res.status(400).send("Sua fkEmpresa está indefinida!");
  } else if (!fkTipoUsuario) {
    return res
      .status(400)
      .send("O campo Tipo de Usuário está vazio ou indefinido!");
  } else if (!senha) {
    ...
    return res
      .status(400)
      .send(
        "O ID do funcionário logado (idFuncionarioServer) não foi retornado!"
      );
  } else {
...
function getUsuariobyID(req, res) {
  ...
  gerenciamentoUsuarioModel
    ...
    .catch(function (erro) {
      console.log(
        "\nHouve um erro ao buscar o usuário! Erro: ",
...
function getTipoUsuario(req, res) {
  ...
  gerenciamentoUsuarioModel
    ...
    .catch(function (erro) {
      console.error(
        "Houve um erro ao listar os tipos de usuário! Erro: ",
        erro.sqlMessage
      );
      res.status(500).json(erro.sqlMessage);
    });
...
function listarFuncionarios(req, res) {
  ...
  console.log(
    `Requisição recebida: página ${pagina}, limite ${limite}, offset ${offset}`
  );

  gerenciamentoUsuarioModel
    .listarFuncionarios(limite, offset)
    .then((resultado) => {
      console.log(`${resultado.length} funcionários retornados`);
      res.status(200).json(resultado);
    })
    .catch((erro) => {
...
function salvarEdicao(req, res) {
  ...
  gerenciamentoUsuarioModel
    .salvarEdicao(nome, email, fkTipoUsuario, senha, idFuncionario)
    .then(function (resultado) {
      if (resultado.affectedRows > 0) {
        ...
        res
          .status(404)
          .json({
            mensagem: "Funcionário não encontrado ou nenhum dado alterado.",
          });
      }
    })
    .catch(function (erro) {
      console.error(
        "Houve um erro ao atualizar o funcionário! Erro: ",
        erro.sqlMessage
      );
...
function ExcluirUsuario(req, res) {
  ...
  if (!idFuncionario) {
    return res
      .status(400)
      .send("ID do funcionário a ser excluído não fornecido na URL!");
  }
  if (!idGerente || !senhaGerente) {
    return res
      .status(400)
      .send("ID do Gerente e Senha são obrigatórios para a exclusão.");
  }
  ...
  gerenciamentoUsuarioModel
    ...
    .then(function (resultadoBusca) {
      ...
      if (senhaGerente === resultadoBusca[0].senha) {
        ...
        gerenciamentoUsuarioModel
          ...
          .then(function (resultadoExclusao) {
            if (resultadoExclusao.affectedRows > 0) {
              console.log(
                `Funcionário de ID ${idFuncionario} excluído com sucesso.`
              );
              ...
              res
                .status(404)
                .send(
                  "Funcionário a ser excluído não encontrado ou já excluído."
                );
            }
          })
          .catch(function (erro) {
            ...
            console.log(
              "\nHouve um erro ao tentar excluir o usuário! Erro: ",
              erro.sqlMessage || erro.message
            );
            res.status(500).json(erro.sqlMessage || erro.message);
          });
        ...
        // 4. Senha incorreta
        console.log(
          "Tentativa de exclusão falhou: Senha do gerente incorreta."
        );
        return res
          .status(403)
          .send("Senha do gerente de autorização incorreta."); // 403 Forbidden
      }
    })
    .catch(function (erro) {
      // 5. Erro durante a busca do Gerente no banco de dados
      console.log(
        "\nHouve um erro ao buscar o Gerente para autenticação! Erro: ",
        erro.sqlMessage || erro.message
      );
      res.status(500).json(erro.sqlMessage || erro.message);
    });
